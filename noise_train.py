...
from noise_model import GPTConfig, GPT
...
# optimizer
optimizer, token_optimizer = model.configure_optimizers(weight_decay, learning_rate, (beta1, beta2), device_type)
if init_from == 'resume':
...
def get_token_lr(it):
    ...
    if it > lr_decay_iters:
        return min_lr
    # 3) in between, use cosine decay down to min learning rate
    decay_ratio = (token_it ) / reset_interval
    assert 0 <= decay_ratio <= 1
    coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio)) # coeff ranges 0..1
    return min_lr + coeff * (learning_rate - min_lr)
...
while True:
    ...
    token_lr = get_token_lr(iter_num) if decay_lr else learning_rate
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            logits, loss = model(X, Y, iteration=iter_num) # NOTE gradual add noise with 1000-iter cycle
    ...
    if grad_clip != 0.0:
        ...
        scaler.unscale_(token_optimizer)
